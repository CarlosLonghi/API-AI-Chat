...
- A new chat mode or endpoint
- Changes crossing both `simple/` and `memory/`
- Schema changes (`schema-postgresql.sql` / `schema-mysql.sql`)
- Introducing authentication/authorization (e.g. replacing `DEFAULT_USER_ID`)
...
- Reference exact files and methods, not vague descriptions ("add `chat_memory.deleted_at`", not "update the database").
