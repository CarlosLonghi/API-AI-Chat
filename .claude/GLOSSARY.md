...
### `existsChat`
`MemoryChatRepository.existsChat(UUID chatId)` — must query `chat_memory`, since that's the table populated at chat-creation time. Querying `spring_ai_chat_memory` instead was a real bug: a brand-new chat has no row there yet (no message has been sent), so `MemoryChatService.createChat` → `sendMessage` → `existsChat` would throw `ChatNotFoundException` on the chat's own creation. Fixed; don't reintroduce by "simplifying" the two tables into one check.

### `chatId` as `UUID`
`MemoryChatController`'s `@PathVariable`, `MemoryChatService`'s parameters, and `MemoryChatRepository.existsChat`/`generateChatId` all use `java.util.UUID`, not `String` — `chat_memory.conversation_id` is `UUID` in Postgres, and the JDBC driver doesn't implicitly cast a `String`/varchar parameter to `uuid` in a `WHERE` comparison (`PSQLException: operator does not exist: uuid = character varying`). Spring converts the path variable to `UUID` automatically; an invalid value in the URL is rejected as `MethodArgumentTypeMismatchException` → 400, before it ever reaches the service/repository. The one place that still takes a `String` is `MemoryChatRepository.getChatMessages`, because `spring_ai_chat_memory.conversation_id` is `VARCHAR(36)` — `MemoryChatService` bridges the two with `chatId.toString()`.
